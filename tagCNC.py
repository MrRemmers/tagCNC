...
sys.path.append(os.path.join(PRGPATH, 'fonts'))
...
from CNC import WAIT, CNC, GCode, Block
import Ribbon
import Pendant
...
from EngravePage  import EngravePage

from Tag import Fixture, NamePlate
...
class Application(Toplevel,Sender):
    def __init__(self, master, **kw):
        ...
        if sys.platform == "win32":
            self.iconbitmap("tagCNC.ico")
        else:
            self.iconbitmap("@%s/tagCNC.xbm"%(Utils.prgpath))
...
#        self.canvas = self.canvasFrame.canvas
        ...
        for ent in self.localtags.tag:
            ...
            self.itemstoEngrave[i].text = StringVar()
            b= Entry(tabTextEntry, textvariable= self.itemstoEngrave[i].text)
            #TODO set the width of Entry relative?
            frameWidth =tabTextEntry.winfo_width
            relX = ent['x0']/self.localtags.perimeter['x1']
            mirrorRelY= ((self.localtags.perimeter['y1'] - ent['y0'])/self.localtags.perimeter['y1'])
            b.place(relx = relX, rely = mirrorRelY)#, width = )
            i+=1
        #endregion

        tabCanvas = Frame(self.notebook)
        #region  --- Canvas ---
        self.canvasFrame = CNCCanvas.CanvasFrame(tabCanvas, self)
        self.canvasFrame.pack(side=TOP, fill=BOTH, expand=YES)
        self.canvas = self.canvasFrame.canvas

        self.notebook.add(tabTextEntry, text= "Enter Text")
        self.notebook.add(tabCanvas , text= "Preview")
        ...
        if errors:
            tkMessageBox.showwarning("tagCNC configuration",
                    "The following pages \"%s\" are found in your " \
                    "${HOME}/.tagCNC initialization " \
                    "file, which are either spelled wrongly or " \
                    "no longer exist in tagCNC"%(" ".join(errors)),parent=self)
        ...
        self.engrave  = Page.frames["Engraving"]
        self.pfont     = Page.frames["Font"]
...
#		self.bind('<Control-Key-h>',	self.replace)
    ...
    def about(self, event=None, timer=None):
        ...
        # -----
        row = 0
        l = Label(frame, image=Utils.icons["tagCNC"],
...
#		tools.selectTab(-1)
    ...
    # Create a new file
    #-----------------------------------------------------------------------
    def newFile(self, event=None, prompt = TRUE):
